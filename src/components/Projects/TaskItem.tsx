...
export interface Task {
  ...
  assignedTo?: string;
  isQuest: boolean;
  questId?: string; // Added questId property
}
...
const TaskItem = ({ 
  ...
  const [questLinked, setQuestLinked] = useState(false);

  useEffect(() => {
  const checkQuestLink = async () => {
    const linked = await isLinkedToQuest(task.id, task.title);
    setQuestLinked(linked);
  };
  ...
  const createQuestFromTask = () => {
    ...
    const saveToBackend = async () => {
          try{
            const res =  await fetch(`${API}/api/questinfo/save-quest`, {
              method: 'POST',
  ...
  const isLinkedToQuest = async (id: string, title: string ): Promise<boolean> => {
  try {
    const response = await fetch(`${API}/api/questinfo/is-linked-quest`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      console.error('Failed to check linked quest');
      return false;
    }

    const data = await response.json();
    console.log(task.id, task.title, data.exists)
    return data.exists
  } catch (error) {
    console.error('Error checking quest linkage:', error);
    return false;
  }
};
