...
const QuestCard = forwardRef<HTMLDivElement, QuestCardProps>(({ quest, isDragging, dragHandleProps }, ref) => {
    ...
    if (isMedievalTheme) {
      switch (quest.status) {
        case QuestStatus.AVAILABLE:
          return (
            <Button 
              className="w-full bg-[#6b4f30] hover:bg-[#5a3f26] text-white border-2 border-[#c0a36e] shadow-md transition-all" 
  ...
  return (
    ...
    >
      {isMedievalTheme ? (
        <Card className={`overflow-hidden transition-all h-[360px] flex flex-col justify-between duration-300 shadow-lg border-2 border-[#c0a36e] rounded-lg bg-[url('/assets/themes/medieval/backgrounds/parchment.jpg')] bg-cover bg-center text-[#4b3621] font-serif ${
